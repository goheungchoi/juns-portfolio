...
vec3 snoise3( vec4 x ){
  float s  = snoise(vec4( x )*0.01);
  float s1 = snoise(vec4( x.y - 19.1 , x.z + 33.4 , x.x + 47.2, x.w )*0.01);
  float s2 = snoise(vec4( x.z + 74.2 , x.x - 124.5 , x.y + 99.4, x.w )*0.01);
  return vec3( s , s1 , s2 );
}

`;

export default glslSimplexNoise;